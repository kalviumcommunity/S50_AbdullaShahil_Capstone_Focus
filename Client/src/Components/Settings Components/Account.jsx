  ...
  useEffect(() => {
    ...
    axios.put(`http://localhost:4000/users/change/${id}`, { password, newPassword })
      .then(response => {
        ...
        setLoading(false);
        reset();
        setPasswordStatus('success'); // Set password status to success
      })
      .catch(error => {
        console.error(error)
        console.log(error.response.data);
        setLoading(false);
        setPasswordStatus('failure'); // Set password status to failure
      });
  };
      ...
      <div className="flex items-center">
        <div className="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-3">
          ...
          <label htmlFor="newPassword">
            <span className="text-sm text-gray-500">New Password</span> {/* Fixed label text */}
            <div className="relative flex overflow-hidden rounded-md border-2 transition focus-within:border-blue-600">
